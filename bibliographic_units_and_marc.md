...
The full Work → Expression → Manifestation → Issue → Item tree for this case has been moved to its own page: **[WEMI Tree — the Venice Haggadot](wemi-tree.qmd)** (Methods → WEMI Tree on the dashboard navbar). The diagram concentrates the visual mass on Expression D (Hebrew + Judeo-Italian), where the 1716 and 1740 Venice nodes split into Issues that WEMI cannot represent, and pins a specific copy at the Item level as the project's scan target.
